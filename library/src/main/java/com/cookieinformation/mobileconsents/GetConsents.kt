...
public class GetConsents(private val applicationContext: Context) :
  ActivityResultContract<Bundle?, Map<UUID, ConsentWithType>>() {
  public override fun createIntent(context: Context, input: Bundle?): Intent =
    Intent(context, PrivacyActivity::class.java)

  override fun parseResult(resultCode: Int, intent: Intent?): Map<UUID, ConsentWithType> {
    return ConsentPreferences(applicationContext).getAllConsentChoicesWithType()
  }
}